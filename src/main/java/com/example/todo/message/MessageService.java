...
public class MessageService {
  ...
  public MessageDto create(Long userId, MessageCreateDto request) {
    ...
    ChatCompletion chatCompletion = callOpenAIWithContext(conversation, request.getModel(), user, inputMessage);
    
    // 5. create ai response message (including tool calls)
    Message outputMessage = createAssistantMessage(
        user, 
        conversation, 
        inputMessage, 
        chatCompletion, 
  ...
  private Message findParentMessage(UUID parentMessageId, UUID conversationId, Integer messageIndex) {
  ...
  private Message createAndSaveUserMessage(User user, Conversation conversation, MessageCreateDto request) {
    Integer messageIndex = getNextMessageIndex(conversation.getId());
    Message parentMessage = findParentMessage(request.getParentMessageId(), conversation.getId(), messageIndex);
    
    Message inputMessage = Message.create(
        user,
        conversation,
        parentMessage,
        messageIndex,
  ...
  private Message createAssistantMessage(
    ...
    List<ContentDto> outputContents = buildOutputContents(chatCompletion, toolCallResult.toolContents);
    
    // 3. create and save assistant message
    Integer messageIndex = getNextMessageIndex(conversation.getId());
    Message outputMessage = Message.create(
        null,
        conversation,
        parentMessage,
        messageIndex,
    ...
    saveToolCallsWithMessage(user, outputMessage, toolCallResult.toolCalls, outputContents, toolCallResult.toolContents);
  ...
  private void updateToolContentWithId(
      ToolContentDto originalContent,
      ToolCall savedToolCall,
      List<ContentDto> outputContents
  ) {
    ToolContentDto updatedContent = new ToolContentDto(
  ...
  private List<ContentDto> buildOutputContents(ChatCompletion chatCompletion, List<ToolContentDto> toolContents) {
  ...
  private List<MessageDto> getConversationContext(UUID conversationId) {
    ...
    List<Message> recentMessages = messageRepository.findByConversationIdOrderByIndexDesc(conversationId, pageable);
  ...
  private String generateConversationTitle(MessageDto inputMessage, String outputText, User user, Message message) {
    String inputText = inputMessage.getContent().stream()
        .filter(content -> content instanceof TextContentDto)
        .map(content -> ((TextContentDto) content).getText())
        .findFirst()
        .orElse("");
    
    String truncatedOutput = outputText.length() > TITLE_PROMPT_MAX_LENGTH 
    ...
    String title = openAIService.chatCompletion(List.of(systemMessage), Model.GPT_4_1_NANO, user, message);
  ...
  private MessageDto buildResponseDto(Message outputMessage, Conversation conversation, boolean isNewConversation) {
  ...
  public MessageDto processToolResult(Long userId, ToolCall toolCall, String toolResultMessage) {
    ...
    Message toolResultMsg = createToolResultMessage(user, conversation, originalMessage, toolResultMessage);
    
    // call openai api and process response
    ChatCompletion chatCompletion = callOpenAIWithContext(conversation, Model.GPT_4_1_NANO, user, toolResultMsg);
    
    // create assistant response message
    Message assistantResponse = createAssistantResponseForToolResult(
        user,
        conversation,
        toolResultMsg,
        chatCompletion
  ...
  private Message createAssistantResponseForToolResult(
    ...
    // process tool calls
    ToolCallResult toolCallResult = processToolCallsForResponse(user, parentMessage, chatCompletion);
  ...
  private List<ContentDto> buildResponseContents(ChatCompletion chatCompletion, List<ToolContentDto> toolContents) {
    List<ContentDto> contents = new ArrayList<>();
